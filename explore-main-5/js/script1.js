...
function loadImageTexture(imageNumber) {
  return new Promise((resolve) => {
    ...
    const path = `assets/images/space3/img${imageNumber}.jpg`;
...
async function createBlock(baseY,yOffset, sectionIndex,blockIndex){
    const blockGeometry = createCurvedPlane(4,2.4, radius,10);
    const imageNumber = (sectionIndex * blocksPerSection + blockIndex) % 30 + 1;
...
// styling 
const lenisContainer = document.documentElement; 

if (lenisContainer.classList.contains("lenis-smooth")) {
    lenisContainer.style.scrollBehavior = "auto"; 
}

if (lenisContainer.classList.contains("lenis-stopped")) {
    lenisContainer.style.overflow = "clip";
}

lenisContainer.querySelectorAll("iframe").forEach(iframe => {
